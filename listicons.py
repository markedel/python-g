...
class ListIcon(ListTypeIcon):
    ...
    def createAst(self):
        ...
        return icon.composeAttrAst(self, ast.List(elts=elts,
                ctx=nameicons.determineCtx(self), lineno=self.id, col_offset=0))

    @staticmethod
    def _stretchedLBracketImage(desiredHeight):
        return icon.yStretchImage(listLBktImage, listLBrktExtendDupRows, desiredHeight)
...
class DictIcon(ListTypeIcon):
    ...
    def createAst(self):
        ...
        if len(self.sites.argIcons) == 1 and self.sites.argIcons[0].att is None:
            # Empty Dict
            return icon.composeAttrAst(self, ast.Dict([], [], lineno=self.id,
                    col_offset=0))
        else:
            for site in self.sites.argIcons:
                if site.att is None:
            ...
            if isDict:
                ...
                return icon.composeAttrAst(self, ast.Dict(keyAsts, valueAsts,
                        lineno=self.id, col_offset=0))
            else:
                eltAsts = [elt.createAst() for elt in elts]
                return icon.composeAttrAst(self, ast.Set(elts=eltAsts, lineno=self.id,
                 col_offset=0))
    ...
    def _stretchedLBraceImage(desiredHeight):
        return icon.yStretchImage(lBraceImage, lBraceExtendDupRows, desiredHeight)
