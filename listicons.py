...
class ListTypeIcon(icon.Icon):
    def __init__(self, leftText, rightText, window, leftImgFn=None,
            rightImgFn=None, closed=True, obj=None, location=None):
...
class ListIcon(ListTypeIcon):
    def __init__(self, window, closed=True, obj=None, location=None):
        if obj is not None:
    ...
    def createAst(self):
        ...
        if self.object is not None:
            ...
            execDataAst = icon.createAstDataRef(self)
...
class TupleIcon(ListTypeIcon):
    def __init__(self, window, noParens=False, closed=True, obj=None, location=None):
    ...
    def createAst(self):
        ...
        if self.isComprehension():
            return icon.composeAttrAst(self, createComprehensionAst(self))
        if self.object is not None and self.compareData(self.object):
            # If this tuple icon faithfully represents its data object, produce an AST
            # referencing the existing data rather than an AST to make a new tuple
            return icon.composeAttrAst(self, icon.createAstDataRef(self))
        if len(self.sites.argIcons) == 1 and self.sites.argIcons[0].att is None:
            elts = []
